...
void initMatrix(std::vector<double>& arr, int size) {
    arr[0] = 10.0;
    arr[size-1] = 20.0;
...
desc.add_options()
    ...
    #pragma acc data copyin(error,last_matrix[0:size*size],matrix[0:size*size])
    {
        while (iter < count_iter && error > accuracy) {
            #pragma acc parallel loop present(matrix,last_matrix)
            for (size_t i = 1; i < size-1; i++) {
            ...
            if ((iter+1)%100 == 0){
                ...
                #pragma acc parallel loop reduction(max:error) present(matrix,last_matrix)
                for (size_t i = 1; i < size-1; i++) {
                    for (size_t j = 1; j < size-1; j++) {
                ...
                #pragma acc update self(error)
                 std::cout << "iter № " << iter << " Error = " << error << " iteration: " << iter << std::endl;
            }
   ...
   std::cout<<"time = "<< time_s << "ms"<< std::endl;
    ...
    saveMatrixToFile(A.arr.data(), size , "matrix.txt");
